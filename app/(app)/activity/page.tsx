  ...
  return (
    <div className="bg-[#111118] border border-[#1E1E2E] rounded-xl px-3 py-2.5 flex items-center gap-2">
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-amber-400">{fav.calories_burned} {t.kcalBurned}</p>
      </div>
      <button
        onClick={() => onLog(fav)}
        disabled={isLogging}
...
function AddActivity() {
  ...
  async function handleLogFavorite(fav: FavoriteActivity) {
    ...
    if (error) {
      toast.error(t.failedToLogActivity)
  ...
  async function handleAnalyze() {
    ...
    try {
      const res = await fetch('/api/analyze-activity', {
        ...
        body: JSON.stringify({ text: description, weightKg, locale }),
      })
      ...
      if (!data.valid) {
        ...
        if (data.error_type === 'not_an_activity') {
          setValidationError(t.notAnActivity)
        } else if (data.error_type === 'too_vague') {
          setValidationError(t.tooVague)
        } else {
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex items-center gap-3 mb-6 pt-2">
        ...
        <h1 className="text-lg font-bold text-[#F8FAFC]">{t.logActivity}</h1>
      </div>

      {/* Favorites */}
      {favorites.length > 0 && (
        <div className="mb-5">
          <p className="text-xs font-semibold text-[#64748B] uppercase tracking-wider mb-2">{t.quickLog}</p>
      ...
      {phase === 'analyzing' && (
        <div className="mt-5 bg-[#111118] border border-amber-500/30 rounded-2xl p-5">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <p className="text-sm font-semibold text-[#F8FAFC]">{t.calculatingCaloriesBurned}</p>
              <p className="text-xs text-[#64748B] mt-0.5">{t.usingMETValues}</p>
            </div>
          </div>
          <div className="space-y-2">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <p className="text-2xl font-bold text-amber-400">
                  ...
                  <span className="text-sm font-normal text-[#64748B] ml-1">{t.kcalBurned}</span>
                </p>
              </div>
              {result.durationMinutes > 0 && (
                <p className="text-sm text-[#64748B] mt-1">{result.durationMinutes} {t.min}</p>
              )}
            </div>
