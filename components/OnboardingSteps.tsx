...
export default function OnboardingSteps({ onComplete }: OnboardingStepsProps) {
  ...
  function isStepComplete() {
    switch (currentStep) {
      ...
      case 1: return !!(formData.age && formData.weight && formData.height)
  ...
  const caloriePreview =
    formData.age && formData.weight && formData.height && formData.gender && formData.activity_level && formData.goal
          ...
          <div>
            ...
            <div className="space-y-2">
              {activityOptions.map(({ value, label, desc }) => (
                <button
                  key={value}
                  onClick={() => setFormData((d) => ({ ...d, activity_level: value }))}
